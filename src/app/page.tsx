...
export default function Dashboard() {
  ...
  const fetchConversations = useCallback(async () => {
    try {
      ...
      if (!res.ok || !Array.isArray(data)) {
        setLoadError(data?.error || "Failed to load conversations");
        setConversations([]);
        return;
      }
      setLoadError(null);
        ...
        (payload) => {
          ...
          if (newMsg.conversation_id === selectedId) {
            ...
            setConversations((current) => {
              const convo = current.find((c) => c.id === newMsg.conversation_id);
              if (convo?.mode === "human") {
                setUnreadIds((prev) => new Set(prev).add(newMsg.conversation_id));
              }
              return current;
            });
          }
  ...
  function getAssistantLabel(msg: Message) {
    if (msg.assistant_source === "human") return "You";
    return "AI";
  }
      ...
      <div className="w-[320px] flex flex-col border-r border-white/[0.06]" style={{ background: "#141414" }}>
        <div className="px-5 py-4 border-b border-white/[0.06]">
          <div className="flex items-center gap-3">
            <div className="w-8 h-8 rounded-lg bg-emerald-500 flex items-center justify-center flex-shrink-0">
              <span className="text-white text-xs font-bold">9D</span>
            </div>
            <div>
              <h1 className="text-sm font-semibold text-white leading-tight">9 Digitals Solutions</h1>
        ...
        {loadError && (
          <div className="mx-4 mt-3 px-3 py-2 rounded-lg bg-red-500/10 border border-red-500/20 text-xs text-red-400">
            {loadError}. Check that Supabase URL and keys in <code className="text-red-300">.env</code> are from the same project.
          </div>
        )}
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          {conversations.map((convo) => {
            ...
            const hasUnread = convo.mode === "human" && unreadIds.has(convo.id);
            return (
              <button
                key={convo.id}
              ...
              >
                ...
                <div className="flex items-center gap-3">
                  <div className="relative flex-shrink-0">
                    <div className="w-10 h-10 rounded-full bg-gradient-to-br from-emerald-600 to-emerald-800 flex items-center justify-center text-white text-xs font-semibold">
                      {getInitials(convo.name, convo.phone)}
                    </div>
                    {hasUnread && (
                      <span className="absolute -top-0.5 -right-0.5 w-3 h-3 bg-amber-400 rounded-full border-2 border-[#141414]" />
                    )}
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between gap-2">
                      <span className={`text-sm truncate ${hasUnread ? "font-semibold text-white" : "font-medium text-white/90"}`}>
                    ...
                    <div className="flex items-center justify-between gap-2 mt-0.5">
                      {convo.last_message ? (
                        <p className={`text-xs truncate ${hasUnread ? "text-white/60" : "text-white/40"}`}>
                          {convo.last_message}
                        </p>
                      ) : (
                        <span />
        ...
        {!selected ? (
          <div className="flex-1 flex flex-col items-center justify-center gap-4">
            <div className="w-16 h-16 rounded-2xl bg-emerald-500/10 flex items-center justify-center">
              <span className="text-emerald-400 text-2xl font-bold">9D</span>
            </div>
            <div className="text-center max-w-sm">
              <p className="text-sm font-medium text-white/50">9 Digitals Solutions</p>
              <p className="text-xs text-white/25 mt-1">
                Software Consultation & Development — Web Apps, AI Chatbots, Business Automation
              </p>
              <p className="text-xs text-white/20 mt-3">Select a conversation to get started</p>
            </div>
          </div>
        ) : (
          <>
            <div className="px-6 py-4 border-b border-white/[0.06] flex items-center justify-between" style={{ background: "#141414" }}>
              ...
              >
                ...
                {selected.mode === "agent" ? "Agent Mode" : "Human Mode"}
              </button>
            </div>
            ...
            >
              {messages.map((msg, i) => {
                ...
                const label = !isUser ? getAssistantLabel(msg) : null;
                return (
                  <div
                    key={msg.id}
                    ...
                    <div className={`flex flex-col ${isUser ? "items-start" : "items-end"} max-w-[65%]`}>
                      ...
                      {showTime && (
                        <p className="text-[10px] text-white/25 mt-1.5 px-1">
                          {!isUser && (
                            <span className={`mr-1 ${label === "You" ? "text-amber-500/60" : "text-emerald-500/60"}`}>
                              {label} ·
                            </span>
                          )}
            ...
            <div className="px-6 py-4 border-t border-white/[0.06]" style={{ background: "#141414" }}>
              <div className="flex items-center gap-3 bg-white/[0.06] rounded-xl px-4 py-2.5 border border-white/[0.06] focus-within:border-emerald-500/40 transition-colors">
                <input
                  ...
                  placeholder={selected.mode === "human" ? "Type your reply..." : "Send a manual message..."}
